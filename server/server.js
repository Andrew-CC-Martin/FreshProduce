...
app.post('/register', (req, res) => {
    let body = _.pick(req.body, ['name','email','password', 'company', 'address', 'delivery_instructions'])
    let user = new User(body);

    user.save().then((user) => {
        return user.generateAuthToken();
    }).then((token) => {        
        res.json(Object.assign({ token }, { _id: user.id, email: user.email, name: user.name, company: user.company, address: user.address, delivery_instructions: user.delivery_instructions }))
...
app.get('/users/:id', (req, res) => {
  ...
  app.patch('/users/:id', (req, res) => {
    ...
    var id = req.params.id;
    var body = _.pick(req.body, ['name', 'email', 'company', 'address', 'delivery_instructions']);
    if (!ObjectID.isValid(id)) {
      return res.status(404).send();
    }
